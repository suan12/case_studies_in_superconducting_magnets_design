...
\end{equation}
式中，$\mathrcal{L}(\alpha,\beta)$是一个无量纲电感参数，仅与由$\alpha,\beta$表示的线圈形状有关。
...
\textbf{``短"线圈}
 ...
 一个``短"$(\beta<0.75)$、``薄壁"(内径$2a_1,\alpha\simeq 1$) 的$N$匝线圈[3.7]：
  \begin{equation}
  \begin{split}
L\simeq&\mu_0a_1N^2\left(\frac{\alpha+1}{2}\right)\bigg(\ln\left\{\left[\frac{2(\alpha+1)}{\beta}\right]\left[1+\frac{\beta^2}{2(1+\alpha^2)}\right]\right\}\\
&-\frac{1}{2}\left[1+\frac{\beta^2}{4(1+\alpha^2)}\right]\bigg)%page108
  \end{split}
  \end{equation}
...
\begin{equation*}
L\simeq\mu_0R\left[\ln\left(\frac{4R}{a}\right)-0.5\right]=\mu_0 R\left[\ln\left(\frac{R}{a}\right)+0.886\right] \tag{3.86c}%page109
\end{equation*}

具有\textit{长方形}截面的环线圈用式3.86c，\textit{圆形}截面的环用式3.80a。
对一个很平($\alpha\gg 1$)的饼式线圈，3.86可以进一步简化为：
\begin{equation*}
L  \simeq  0.5\mu_0a_1\alpha N^2\left\{\ln\left(\frac{25}{6}\right)-\frac{101}{288}\right\}=0.538\mu_0a_1\alpha N^2\approx 0.5\mu_0a_2N^2 \tag{3.86'''}%page109
\end{equation*}
...
M_{RS}=\frac{\mu_0}{2}\left(\frac{N_RN_S}{2b_S}\right)\eft\{\frac{2\pi(a_Ra_S)^2b_S}{\rho^3}\left(1+\frac{b_S}{\rho}\right)
-(a_R-a_S)^2[\prod(c^2,k_S)-\prod(c^2,k_S)]\right\}%page113
  \end{equation}
...
&\left\{[(a_R+a_S)^2+b_S^2][K(\kappa)-E(\kappa)]-\Upsilon(c^2,k)\right\}
\end{split}
\end{equation}
式中，$k$为：
  \begin{equation*}
\kappa^2=\frac{4a_Ra_S}{(4a_R+a_S)^2+b_S^2}
\end{equation*}
