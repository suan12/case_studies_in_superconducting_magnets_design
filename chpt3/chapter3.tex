...
H_z(z,0)&=\frac{H_z(0,0)}{(1+(z/z)^2)^{3/2}} \qquad\mbox{(\quad 以中心处磁场$H_z(0,0)$表示)}
\end{align}
\end{subequations}
专题1将以实例进行进一步的讨论。
...
$K_\theta [\mathrm{A/m}]$计算）。$(0,0)$处的磁感应强度的z分量为
...
绕组内部的$B_z$是$B_0$，外部是$0$，在管壁厚度$\delta$方向线性正比于$r$衰减。所以，平均磁感应强度$\bar{H}_z$是$B_0/2$。因而，r方向施于线圈上的平均Lorentz力密度为
...
H_z(x)&=A_0^0-(3/2A_2^0-15A_2^2)x^2+(15/8 A_4^0-105/2 A_4^2+945A_4^4)x^4+\cdots
\end{align}
\end{subequations}
...
F(\alpha,\beta)E_8(\alpha,\beta)=&-\frac{r^3}{896\beta^7}\left[\frac{16 r^{12}+120 r^{10}\beta^2+390r^8\beta^4+715r^6\beta^6+1080r^4\beta^8-1008r^2\beta^{10}+1344\beta^{12}}{(\alpha^2+\beta^2)^{7.5}}\right]\bigg|_{r=1}^{r=\alpha}
\end{align}
\end{subequations}
...
一般来说(但也不全是)，$z$轴向磁场$H_z(z)$的均匀性要比z轴法平面上的均匀性更好。故，我们仅考虑z轴向磁场，并给出其一般形式：
\begin{subequations}
	\begin{align}
...
\begin{equation}
\begin{split}
F_{zA}(0)=&\frac{\mu_0}{2}[(\frac{N_A I_A}{2b_A})^2\times \{{2b_A\sqrt{4a_A^2+b_A^2}[K(k_{bA})-E(k_{bA})}]-2b_A\sqrt{4a_A^2+4b_A^2}[K(k_{2bA}-E(k_{2bA}]\}\\
...
\begin{equation*}
F_{zB}(0)\simeq-\frac{\mu_0}{2}(\frac{N_BI_B}{2b_B})^2\pi a_B^2 \tag{3.59'}
\end{equation*}
...
\begin{subequations}
	\begin{align}
\mathcal{L}(\alpha,\beta)\sim&\frac{\pi\alpha}{2(\beta+0.5)} \qquad(\mbox{for } \beta\rightarrow 1 from \beta>1)\\ 
...
\begin{equation*}
  L=\mu_0a_1N^2(\frac{\pi}{2\beta})[1-\frac{2}{3\pi\beta}+\frac{1}{8\beta^2}...] \tag{3.84'}%page108
\end{equation*}

b.一个“很长”($\beta\gg 1$)、“薄”($\alpha \simeq 1$)，方程3.84b成为：
\begin{equation*}
  L=\mu_0a_1N^2(\frac{\pi}{2\beta}) \tag{3.84''}%page108
\end{equation*}
