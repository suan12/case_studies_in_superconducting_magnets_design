...
\begin{figure}[htbp]
	\centering
	\includegraphics[scale=0.5]{chpt3/figs/fig3.31.eps}
...
\begin{align*}
\ln[(a_{2}-x)^{2}+(c-y)^{2}]=&\ln\left[(a_{2}^{2}+c^{2})(1+\frac{x^{2}+y^{2}-2a_{2}x-2cy}{a_{2}^{2}+c^{2}})\right]\\
...
当上述积分在螺绕环外的闭合曲线上进行时，由于螺绕环磁体外部无净电流，故$H_\varphi(r)=0,\ B_\varphi(r)=0$。

b) 图3.40给出了一个单环，一个力微元$d\vec{F_L}$作用在它上面的一个线微元$d\vec{s}$上，$r$方向的力微元为$dF_{Lr}$。
$d\vec{F_L}$可以写为：
\begin{equation*}
